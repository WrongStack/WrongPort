...
- `ports`: sonucu belirtilen portlarla sınırlar.
...
npm test             # vitest: 5 dosya / 42 birim test (ayrıntı: aşağıdaki Testler)
...
```bash
npm test            # vitest run — 5 dosya / 42 test (~1.5 sn)
```
...
| `GET /api/processes?all=1` | Snapshot (JSON); `only`, `ports` sorgu parametreleri opsiyonel; geçersiz `only` deseni → 400 + mesaj |
| `POST /api/kill` | Body: `{"pid": 1234, "force": false}` |
